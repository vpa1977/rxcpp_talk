...
void SamplePublish()
{
    ...
    // connectable
    {
        auto keys = rx::observable<>::range(1,10).publish();
        keys.subscribe([](int val) {
            std::cout << val << std::endl;
        });
        auto next = keys.map([](int val) {
            return val + 1;
        });
        auto filtered = next.filter([](int val) { return val > 5; });
        std::cout << "done " << std::endl;
        keys.connect();
    }
...
{
    {
        ...
        auto keys = rx::observable<>::range<int>(0, 10);
        auto willHaveToCallSaul = keys.filter([](int val) {return val == 3; }).
            map([=](int val) {  return val + 1; });
        willHaveToCallSaul.subscribe([=](int) {/* call saul*/ });
        auto saulResponse = keys.filter([=](int val) { return val > 5; });
        willHaveToCallSaul.zip([](int a, int b) {
            return std::make_tuple(a, b);
        }, saulResponse).subscribe([](std::tuple<int,int> val)
        {
              std::cout << "finish " << std::get<0>(val) << 
